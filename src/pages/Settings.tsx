/**
 * src/pages/Settings.tsx — Firestore profile settings page
...
export default function Settings() {
  ...
  const fetchProfile = async () => {
    ...
    try {
      ...
      console.error('[Settings] fetchProfile:', err);
  ...
  if (loading) {
    return (
      <AppShell contentWidth="narrow">
        <PageHeader title={<Skeleton className="h-8 w-52" />} description={<Skeleton className="h-4 w-64" />} back variant="sticky" />
  ...
  return (
    <AppShell contentWidth="narrow">
      <PageHeader title="Account Settings" description="Manage your personal information" back action={saveAction} variant="sticky" />
      <div className="space-y-6 pb-6">
        <Card>
          ...
          <CardContent className="space-y-4">
            ...
            <div className="space-y-2">
              ...
              <Input id="display_name" value={displayName} onChange={(e) => setDisplayName(e.target.value)} placeholder="Enter your display name" />
            </div>
            <div className="space-y-2">
              <Label htmlFor="phoneNumber">Phone Number</Label>
              <InternationalPhoneInput value={phoneNumber} onChange={(val) => setPhoneNumber(val ?? '')} country={getCountryCode(country)} />
            </div>
            <div className="space-y-2">
        ...
        <Card>
          <CardHeader><CardTitle>Developer &amp; Testing</CardTitle></CardHeader>
          <CardContent className="space-y-3">
            <Button variant="outline" className="w-full justify-start" onClick={() => navigate('/test-push')}>Test Firebase Push Notifications</Button>
            <Button variant="outline" className="w-full justify-start" onClick={() => navigate('/test-onesignal')}>Test OneSignal Push Notifications</Button>
            <Button variant="outline" className="w-full justify-start" onClick={() => navigate('/test-emails')}>Test Email Notifications</Button>
          </CardContent>
        </Card>
      </div>
    </AppShell>
