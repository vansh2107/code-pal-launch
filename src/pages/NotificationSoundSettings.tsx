...
interface NotificationPref { enabled: boolean; sound: string; }
...
function getDefaultPrefs(): NotificationPreferences {
  ...
  NOTIFICATION_TYPES.forEach((t) => { d[t.key] = { enabled: true, sound: "default" }; });
  return d;
}

function migratePreferences(raw: unknown): NotificationPreferences {
  if (!raw || typeof raw !== "object") return getDefaultPrefs();
  const result: NotificationPreferences = {};
  for (const type of NOTIFICATION_TYPES) {
    const val = (raw as Record<string, unknown>)[type.key];
...
export default function NotificationSoundSettings() {
  ...
  useEffect(() => { if (user) fetchPreferences(); }, [user]);
  ...
  const fetchPreferences = async () => {
    ...
    try {
      ...
      if (snap.exists()) {
        setPreferences(migratePreferences(snap.data().notificationSounds));
      }
    } catch (err) {
      console.error('[NotificationSoundSettings] fetchPreferences error:', err);
    } finally {
      setLoading(false);
    }
  };
  ...
  const savePreferences = useCallback(async (updated: NotificationPreferences) => {
    ...
    try {
      await setDoc(
        ...
        { notificationSounds: updated as Record<string, unknown>, updatedAt: new Date().toISOString() },
        { merge: true },
      );
    } catch {
  ...
  const handleToggle = useCallback(async (type: NotificationType, enabled: boolean) => {
    ...
    toast({ title: enabled ? "Notification enabled" : "Notification muted", description: NOTIFICATION_TYPES.find(t => t.key === type)?.label });
  }, [preferences, savePreferences]);

  const handleSoundSelect = useCallback(async (soundId: string) => {
    ...
    setPickerOpen(false);
    await savePreferences(updated);
    toast({ title: "Sound updated", description: `${NOTIFICATION_TYPES.find(t=>t.key===activeType)?.label} → ${AVAILABLE_SOUNDS.find(s=>s.id===soundId)?.label ?? soundId}` });
  }, [activeType, preferences, savePreferences]);

  const handlePlay = useCallback((id: string, frequency: number, pattern: readonly number[]) => {
    setPlayingId(id);
  ...
  const openPicker      = (type: NotificationType) => { setActiveType(type); setPickerOpen(true); };
  const getSoundLabel   = (soundId: string) => AVAILABLE_SOUNDS.find(s => s.id === soundId)?.label ?? "Default";
    ...
    <div className="min-h-screen page-bg flex flex-col w-full" style={{ paddingBottom: 'calc(var(--nav-height) + var(--safe-area-bottom) + var(--fab-gap) + 32px)' }}>
      ...
      <main className="flex-1 px-4 py-6 w-full max-w-2xl mx-auto space-y-4">
        <div className="bg-card rounded-2xl border border-border/50 shadow-sm overflow-hidden">
          {NOTIFICATION_TYPES.map((type, index) => {
            ...
            return (
              <div key={type.key} className={cn("p-4", index < NOTIFICATION_TYPES.length - 1 && "border-b border-border/30")}>
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center justify-between mb-1">
                      ...
                      <Switch checked={pref.enabled} onCheckedChange={(checked) => handleToggle(type.key, checked)} />
                    </div>
                    <p className="text-xs text-muted-foreground mb-2">{type.description}</p>
                    <button
                      onClick={() => openPicker(type.key)}
                      disabled={!pref.enabled}
                      className={cn("flex items-center gap-2 text-xs rounded-lg px-2.5 py-1.5 transition-colors", pref.enabled ? "bg-accent/10 text-primary hover:bg-accent/20 cursor-pointer" : "bg-muted/50 text-muted-foreground/50 cursor-not-allowed")}
                    >
        ...
        <Button variant="outline" className="w-full" onClick={handleResetAll} disabled={saving}>
          <RotateCcw className="h-4 w-4 mr-2" />Reset All to Default
        </Button>
      </main>
      ...
      <Dialog open={pickerOpen} onOpenChange={setPickerOpen}>
        <DialogContent className="max-w-sm mx-auto">
          <DialogHeader>
            <DialogTitle className="text-lg">{NOTIFICATION_TYPES.find(t => t.key === activeType)?.label ?? "Select Sound"}</DialogTitle>
          </DialogHeader>
          <ScrollArea className="max-h-[60vh]">
            <div className="space-y-1 py-2">
              {AVAILABLE_SOUNDS.map((sound) => {
                ...
                return (
                  <div key={sound.id}
                    className={cn("flex items-center justify-between rounded-xl px-3 py-3 transition-colors cursor-pointer", isSelected ? "bg-primary/10 border border-primary/20" : "hover:bg-accent/5")}
                    onClick={() => handleSoundSelect(sound.id)}
                  >
                    <div className="flex items-center gap-3">
                      <div className={cn("w-5 h-5 rounded-full border-2 flex items-center justify-center transition-colors", isSelected ? "border-primary bg-primary" : "border-muted-foreground/40")}>
                        {isSelected && <Check className="h-3 w-3 text-primary-foreground" />}
                      </div>
                      <span className={cn("text-sm", isSelected ? "font-medium text-foreground" : "text-muted-foreground")}>{sound.label}</span>
                    </div>
                    {sound.id !== "none" && (
                      <Button variant="ghost" size="icon" className="h-8 w-8 shrink-0"
                        onClick={(e) => { e.stopPropagation(); handlePlay(uniqueId, sound.frequency, sound.pattern); }}>
          ...
          <div className="pt-2 border-t border-border/50">
            <Button variant="outline" className="w-full"
              onClick={() => toast({ title: "Coming soon", description: "Custom sound import will be available in a future update." })}>
              <Upload className="h-4 w-4 mr-2" />Import Custom Sound
            </Button>
          </div>
        </DialogContent>
      </Dialog>
